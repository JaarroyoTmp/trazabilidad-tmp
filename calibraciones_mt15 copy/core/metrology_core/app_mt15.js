...
function ensureInstrumentSelectorUI() {
  ...
  panel.innerHTML = `
    ...
    <div class="row">
      ...
      <button id="tmpInstrumentSearchButton" type="button" class="btn">Buscar</button>
      <button id="tmpInstrumentCloseButton" type="button" class="btn secondary">Cerrar</button>
    </div>
  ...
  rows.innerHTML = list.map(inst => {
    ...
    return `
      <tr>
        ...
        <td>
          <button type="button" class="btn secondary tmp-select-instrument" data-codigo="${inst.codigo || ""}">
            Seleccionar
          </button>
        </td>
      </tr>
...
function renderResults() {
  ...
  const dictamenFinal = safeStatus(r.global?.resultado_operativo || r.resultado_operativo || r.global?.status);
  const evaluacionIlac = safeStatus(r.global?.resultado_auditoria || r.resultado_auditoria || r.global?.status);
  const motivoDictamen = r.global?.motivo_operativo || "";

  const puntosIlacIndeterminados = r.puntos.filter(p => getDecisionAudit(p.decision) === "INDETERMINADO").length;
  const puntosIlacNoAptos = r.puntos.filter(p => getDecisionAudit(p.decision) === "NO_APTO").length;
  ...
  let html = `
    <div class="banner ${bannerClass(dictamenFinal)}">
      <strong>Dictamen final TMP:</strong> ${dictamenFinal}
      <br>
      <span class="mini">
        Este es el resultado válido para uso del equipo en taller/laboratorio.
        ${motivoDictamen ? `<br>${motivoDictamen}` : ""}
      </span>
    </div>

    <div class="banner mt" style="background:#eff8ff;border-color:#b2ddff;color:#184e77;">
      <strong>Evaluación metrológica:</strong> ILAC-G8 / ISO 14253 aplicada como soporte técnico del dictamen final.
      <br>
      <span class="mini">
        ${evalText}<br>
        U(k=2) máxima: ${fmtUm(maxU, 3)} · Error absoluto máximo: ${fmtUm(maxErrorAbs, 3)}.
      </span>
    </div>
  `;
  ...
  for (const p of r.puntos) {
    ...
    const evalLabel = auditoria === "INDETERMINADO"
      ? "Incertidumbre considerada"
      : auditoria === "NO_APTO"
        ? "Revisión técnica necesaria"
        : "Conforme técnicamente";
    const evalStyle = auditoria === "NO_APTO"
      ? "background:#fef3f2;color:#b42318;border:1px solid #fecdca;"
      : "background:#eff8ff;color:#184e77;border:1px solid #b2ddff;";
    ...
    html += `
      <tr>
        ...
        <td><span class="pill" style="${evalStyle}">${evalLabel}</span></td>
        <td class="mini">
          ${p.decision.motivo_operativo || p.decision.motivo || p.decision.reason || "-"}
          <br>
          <strong>Regla:</strong> ILAC-G8 / ISO 14253 como evaluación técnica; dictamen final TMP mostrado en columna anterior.
        </td>
      </tr>
    `;
  }
    ...
    <div class="card mt" style="background:#071426">
      ...
      <div class="table-box mt">
        <table>
          ...
          <tbody>
            <tr>
              <td><strong>Dictamen final TMP</strong></td>
              <td><span class="pill ${statusClass(dictamenFinal)}">${dictamenFinal}</span></td>
              ...
              <td class="mini">La evaluación ILAC-G8 queda documentada como soporte técnico, no como segundo resultado.</td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  html += `
    <div class="card mt" style="background:#071426">
      <h3>Detalle técnico para auditoría</h3>
      <p class="mini">
        La tabla siguiente conserva la trazabilidad de patrón, certificado, corrección e incertidumbre utilizada.
        Si un punto cae en zona de incertidumbre, se documenta aquí como información técnica, sin duplicar el dictamen final.
      </p>
      <div class="table-box mt">
        <table>
          <thead>
