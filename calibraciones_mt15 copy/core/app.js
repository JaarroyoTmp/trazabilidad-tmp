...
function pintarInstrumento(){
  ...
  if (prox && hoy > prox) {
    cl = "bad";
    txt = "FUERA";
  } else if (prox && (+prox - +hoy) < 1000 * 60 * 60 * 24 * 30) {
    cl = "warn";
    txt = "PRÓXIMA";
  }

  est.className = "status " + cl;
  est.textContent = txt;

  const familiaMT15 = detectarFamiliaMT15(ins);
  const criterioMT15 = obtenerCriterioMT15(familiaMT15);

  console.log("MT15 detectado:", familiaMT15, criterioMT15);

  let mt15Box = document.getElementById("mt15DetectadoBox");

  if (!mt15Box) {
    mt15Box = document.createElement("div");
    mt15Box.id = "mt15DetectadoBox";
    mt15Box.className = "card";
    mt15Box.style.marginTop = "12px";

    const instrumentoCard = document.querySelector(".card");
    if (instrumentoCard) {
      instrumentoCard.appendChild(mt15Box);
    }
  }

  mt15Box.innerHTML = `
    <div class="row">
      <div class="pill success">MT15 DETECTADO</div>
      <div class="chip">${familiaMT15}</div>
    </div>

    <p class="mini" style="margin-top:8px">
      <strong>Procedimiento:</strong> ${criterioMT15.procedimiento}<br>
      <strong>Tipo:</strong> ${criterioMT15.tipo}<br>
      <strong>Referencia:</strong> ${criterioMT15.referencia}<br>
      <strong>Regla de decisión:</strong> ${criterioMT15.regla_decision}<br>
      <strong>Puntos mínimos:</strong> ${criterioMT15.puntos_minimos} ·
      <strong>Repeticiones mínimas:</strong> ${criterioMT15.repeticiones_minimas}
    </p>
  `;
}
...
/* ========= GUARDAR TODO EN SUPABASE SIN DEPENDER DEL BACKEND ========= */
$("#btnGuardarTodo").addEventListener("click", async () => {
  ...
  if (!state.plan.length) {
    alert("No hay plan de calibración definido.");
    return;
  }
  ...
  try {
    const calibracionId = await guardarHistoricoCalibracion(
      certJSON,
      null
    );
    ...
    if (!ok) {
      alert(
        "La calibración se guardó, pero no se pudieron actualizar las fechas del instrumento."
      );
      return;
    }
    ...
    alert(
      ...
      "Calibración ID: " + calibracionId
    );

  } catch (err) {
    console.error("Error guardando calibración completa:", err);
    alert(
      "Error guardando calibración completa en Supabase. Mira la consola."
    );
  }
});

/* ========= GUARDAR HISTÓRICO COMPLETO EN SUPABASE ========= */
async function guardarHistoricoCalibracion(certJSON, certificadoBackend) {
  await ensureSupabase();

  if (!supabase) {
    throw new Error("Supabase no disponible para guardar histórico.");
  }

  const ins = certJSON.instrumento || {};
  const condiciones = certJSON.condiciones || {};
  const resumen = certJSON.resumen_global || {};
  const patrones = certJSON.patrones || [];
  const bloques = certJSON.bloques || [];

  const fechaCal = condiciones.fecha_calibracion || new Date().toISOString();
  const fechaProx =
    certJSON.firma?.proxima_calibracion ||
    ins.fecha_proxima_calibracion ||
    null;

  const patronesUsados = patrones
    .map(p => p.codigo || p.id || "")
    .filter(Boolean);
  ...
  const { data: calData, error: calError } = await supabase
    .from("calibraciones")
    .insert({
      instrumento_id: state.instrumento?.id || ins.id || null,
      fecha_ultima: fechaCal,
      fecha_proxima: fechaProx,
      fecha_calibracion: fechaCal,
      laboratorio: "TMP",
      certificado_url: certificadoBackend?.pdf_url || null,
      observaciones: condiciones.observaciones || "",
      codigo: ins.codigo || "",
      conforme: resumen.decision === "APTO",
      resultado: resumen.decision || "",
      patrones_usados: patronesUsados,
      operador: condiciones.operario || certJSON.firma?.nombre || "",
      incertidumbre: Number(resumen.max_abs_error_plus_u_um || 0),
      estado_final: resumen.decision || ""
    })
    .select("id")
    .single();

  if (calError) {
    console.error("Error guardando calibraciones:", calError);
    throw calError;
  }

  const calibracionId = calData.id;
  ...
  for (const patron of patrones) {
    const { error } = await supabase
      ...
      .insert({
        ...
        factor_aplicacion: patron.codigo || patron.descripcion || ""
      });

    if (error) {
      console.error("Error guardando calibracion_patrones:", error);
      throw error;
    }
  }
  ...
  for (const bloque of bloques) {
    ...
    for (const punto of puntos) {
      const tolerancia = Number(punto.T || 0);

      const { data: puntoData, error: puntoError } = await supabase
        .from("calibracion_puntos")
        .insert({
          calibracion_id: calibracionId,
          punto_ordinal: ordinalGlobal,
          nominal: punto.nominal ?? null,
          unidad: ins.unidad_base || "mm",
          media: punto.media ?? null,
          error: punto.error ?? null,
          tolerancia_min: tolerancia ? -Math.abs(tolerancia) : null,
          tolerancia_max: tolerancia ? Math.abs(tolerancia) : null,
          conforme: punto.decision === "APTO",
          extras: {
            bloque_id: bloque.id || null,
            bloque_tipo: bloque.tipo || "",
            lado: bloque.lado || "",
            referencia_utilizada: punto.referencia_utilizada ?? null,
            correccion_patron: punto.correccion_patron ?? null,
            caracteristica: punto.caracteristica || "",
            sigma: punto.sigma ?? null,
            U: punto.U ?? null,
            T: punto.T ?? null,
            decision: punto.decision || "",
            familia_mt15: condiciones.familia_mt15 || null,
            criterio_mt15: condiciones.criterio_mt15 || null,
      ...
      if (puntoError) {
        console.error("Error guardando calibracion_puntos:", puntoError);
        throw puntoError;
      }

      const puntoId = puntoData.id;
      const repeticiones = punto.repeticiones || [];
      ...
      for (let i = 0; i < repeticiones.length; i++) {
        ...
        if (lecturaError) {
          console.error("Error guardando calibracion_lecturas:", lecturaError);
          throw lecturaError;
        }
      }
  ...
  const { error: certError } = await supabase
    ...
    .insert({
      calibracion_id: calibracionId,
      numero: certificadoBackend?.numero || null,
      regla_decision: certJSON.regla_decision || "ILAC-G8",
      decision_global: certJSON.decision_global || resumen.decision || "",
      certificado_pdf_url: certificadoBackend?.pdf_url || null,
      datos: certJSON
    });
